...
export default function ProductsPage() {
  ...
  useEffect(() => {
    ...
    // Apply sorting (using base price from first variant)
    if (sortBy === "price-low") {
      result.sort((a, b) => {
  ...
  const renderProductCard = (product) => (
    ...
    >
      <Link href={`/products/${product._id}`}>
        <div className="relative h-64 md:h-72 bg-cover bg-center group-hover:scale-110 transition-transform duration-700 cursor-pointer"
          style={{
            backgroundImage: `url('${
              product.image ||
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-8">
          <div>
            ...
            <p className="text-xl text-gray-300 mt-2">
              {filteredProducts.length} delicious {filteredProducts.length === 1 ? 'item' : 'items'} available
            </p>
          </div>
